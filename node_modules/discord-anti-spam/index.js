...
class AntiSpam extends Events.EventEmitter {
  constructor(options) {
    ...
    //this.kickThreshold = options.kickThreshold || 4;
    this.maxInterval = options.maxInterval || 2000;
    this.warnMessage = options.warnMessage || "{@user}, Arrête de spam s'il te plaît.";
    this.banMessage = options.banMessage || "**{user_tag}** a été banni pour spam..";
  ...
  message(message) {
    ...
    let raison = "Ban pour spam !"

    let banEmbedSpam = new Discord.RichEmbed()
    .setTitle("Ban d'un personne")
      .setDescription("~Ban~")
      .setThumbnail("https://cdn.leagueofgraphs.com/img/lolfr.jpg")
      .setColor("#bc0000")
      .addField("Banned User", `${message.member} with ID ${message.id}`)
      //.addField("Banned By", `<@${message.author.id}> with ID ${message.author.id}`) // Bloc embed pour channel de log
